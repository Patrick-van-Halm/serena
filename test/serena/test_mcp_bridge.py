...
def test_bridge_heartbeat_does_not_self_exit_when_idle() -> None:
    bridge = SerenaMCPBridge.__new__(SerenaMCPBridge)
    bridge.project_root = "/repo"
    bridge.context = SerenaAgentContext.from_name("codex")
    bridge.session_id = "chat-idle"
    bridge.client = MagicMock()
    ...
    bridge._heartbeat_stop = StopAfterHeartbeat()
    bridge._heartbeat_loop()

    bridge.client.heartbeat_bridge.assert_called_once_with(
        project_root="/repo",
        context="codex",
